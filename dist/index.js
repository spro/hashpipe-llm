...
exports._internal = exports.models = exports.structured = exports.json = exports.llm = void 0;
...
function resolveModel(ctx, override) {
    const rawModel = override || getVar(ctx, "llm_model") || process.env.LLM_MODEL || DEFAULT_MODEL;
    ...
    if (!provider) {
        throw new Error("could not infer provider for model " +
            model +
    ...
    return {
        provider,
        id: model,
        model: provider === "anthropic" ? (0, anthropic_1.anthropic)(model) : (0, openai_1.openai)(model),
    };
}
function buildPrompt(inp, args) {
...
function complete(inp, args, ctx, extraSystem, cb) {
    try {
...
function isStructuredSpec(value) {
    return (value != null &&
        typeof value === "object" &&
        (Array.isArray(value) || !("quoted" in value)));
}
function splitStructuredArgs(args) {
...
const llm = (inp, args, ctx, cb) => {
    complete(inp, args, ctx, null, cb);
};
exports.llm = llm;
// llm.json <prompt...> - response is parsed JSON, ready for piping.
const json = (inp, args, ctx, cb) => {
    complete(inp, args, ctx, "Respond with ONLY valid JSON (an object, array, string, number, or boolean). No prose, no markdown fences.", (err, text) => {
        if (err != null)
            return cb(err);
...
const structured = (inp, args, ctx, cb) => {
    try {
        const flags = extractModelFlag(args);
        ...
        const structuredOutput = (0, structured_1.compileStructuredOutput)(schemaSpec);
        const model = resolveModel(ctx, flags.model);
        (0, ai_1.generateText)({
            model: model.model,
            maxOutputTokens: MAX_TOKENS,
...
function formatNoObjectError(err) {
...
const models = (inp, args, ctx, cb) => {
    try {
        const provider = normalizeProvider(args[0] || getVar(ctx, "llm_provider"));
        if (provider)
            return cb(null, KNOWN_MODELS[provider]);
...
exports._internal = {
    buildPrompt,
    compileStructuredOutput: structured_1.compileStructuredOutput,
    extractModelFlag,
    inferProvider,
    normalizeProvider,
    splitStructuredArgs,
